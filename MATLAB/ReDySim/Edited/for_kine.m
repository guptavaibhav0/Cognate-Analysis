...
function [so, sc, vc, tt, st, cent]=for_kine(th, dth, ~, ~, ~, ~, ~, ~, ~, ~, ~)
...
for i=1:n
    if bt(i)==0 %When parent of the link is ground link
        ...
        sc(:,i)=so(:,i)+Qf(:,:,i)*di;
    %assuming the centre of mass of each link is at the mid point of
    %the link 
        st(:,i)=so(:,i)+Qf(:,:,i)*2*[di(1);0;0];
%     else, if the mass of the link is concentrated at the end - this is
%     only for the cases of links 6 and 9 to enable force input at the
%     stationary point of the mechanism
        if i==6||i==9
